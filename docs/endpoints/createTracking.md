# Register a shipment for tracking (deprecated alias)

`POST /trackings`

- Operation: `createTracking`
- Tags: Tracking
- Product: tracking
...
Deprecated alias of `createShipment` (`POST /shipments`) with the same parameters, body and answer. Do not use it in new integrations; it is scheduled for removal on 2027-03-31.
...
```bash
curl -X POST "https://api-sandbox.connectix.bg/trackings?integration=npu5K6DCMcsmLyDjLNpSkc8szq8iUSlY6Ga96vJjxsKOYWiAILbvQp9qXkKKGIfl" \
